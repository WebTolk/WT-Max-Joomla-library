...
5. автоматически вычисляет tag релиза из версии апстрима `webtolk/max`
6. при ручном запуске `workflow_dispatch` собирает пакет и прикрепляет итоговый ZIP к GitHub Release этой upstream-версии
7. при tag run использует тот же контракт: версия релиза и имя ZIP определяются по апстриму
